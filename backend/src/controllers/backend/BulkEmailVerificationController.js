...
exports.uploadBulkEmails = async (req, res) => {
  try {
    ...
    const newBulkJob = new BulkVerification({ job_id, user_id: req.user.id, type: "bulk"  });
  ...
  exports.startBulkVerification = async (req, res) => {
    try {
      ...
      return res.status(200).json(Response.success('Email id verification started', response.data));
...
exports.verifySingleEmail = async (req, res) => {
  try {
    ...
    const existingVerification = await EmailVerification.findOne({ email, type: "single" });

    if (existingVerification) {
      const responseCredits = await axios.get(`https://api.bouncify.io/v1/info?apikey=${API_KEY}`);
    ...
    const response = await axios.get(`https://api.bouncify.io/v1/verify?apikey=${API_KEY}&email=${email}`);
    const data = response.data;
    ...
    console.log("Updated credit response:", responseCredits.data.credits_info.credits_remaining); // Debugging
    ...
    const updatedCreditInfo = await CreditInfo.findOneAndUpdate(
      { user_id: req.user.id },
      {
        ...
        $set: { credits_remaining: responseCredits.data.credits_info.credits_remaining }, // Update remaining credits
      },
